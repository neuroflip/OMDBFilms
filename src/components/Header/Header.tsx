...
const Header = () => {
    const [ displayName, contentClass, isOpen, menuOnClick, onLogoutClick ] = useHeader();
    return (<>
        <header className="grid items-center align-start grid-cols-2 grid-rows-auto px-4 py-4 fixed top-0 left-0 w-full">
            ...
            <button onClick={ menuOnClick } className="text-secondary text-2xl justify-self-end max-h-5 sm:hidden">{ isOpen ? "x" : "☰" }</button>
            ...
            <NavLink to="/" onClick={ onLogoutClick } className="link block sm:inline">logout</NavLink>
            </div>
        </header>
    </>);
}
